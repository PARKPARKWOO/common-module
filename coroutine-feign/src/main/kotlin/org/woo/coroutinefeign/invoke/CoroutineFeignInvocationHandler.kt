...
class CoroutineFeignInvocationHandler(
    ...
    override fun invoke(proxy: Any, method: Method, args: Array<out Any>?): Any {
        ...
        val path = method.getAnnotation(Get::class.java)?.path
        ...
        val serviceName = method.declaringClass.getAnnotation(CoroutineFeignClient::class.java)?.serviceName
        ...
        return runBlocking {
            when (httpMethod) {
                ...
                "POST" -> adapter.handlePostRequest(
                    appName = serviceName,
                    path = path,
                    headers = headers,
                    body = body,
                ...
                "PATCH" -> adapter.handlePatchRequest(
                    appName = serviceName,
                    path = path,
                    headers = headers,
                    body = body,
                ...
                "DELETE" -> adapter.handleDeleteRequest(
                    appName = serviceName,
                    path = path,
                    headers = headers,
                    body = body,
    ...
    private fun extractBody(method: Method, args: Array<out Any>?): Any? {
        method.parameters.forEachIndexed { index, parameter ->
    ...
    private fun extractQueryParams(method: Method, args: Array<out Any>?): Map<String, String?> {
